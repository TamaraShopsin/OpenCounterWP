	...
	var Thinbox = function(element,options) {
		...
		this.setupModal = function(vars) {
			...
			if(vars.isImage && self.settings.showCloseButton) {
				var imageCloseButton = '<a class="jsThinboxClose '+self.settings.thinboxModalCloseButton+'">X</a>';
		...
		this.showModal = function(element,url) {
			...
			if(href.substr(0,1)=="#") {
				...
				var thinboxContent = $("<div style='betabox'><div style='betabox2'>BETA VERSION<input type='button' value='X' onclick='ThinBox.remove()' style='betax'/></div><br/><br/><div style='betabox3'>Welcome to Open Counter, an experimental (‘beta’) site built for the City of Santa Cruz by Code for America.</br></br>PLEASE BE AWARE – this is a test website and may contain inaccuracies.</br></br>Your suggestions will help us make this site better, so if you have any comments please click on the tab to the left and leave feedback.</div></div>");
